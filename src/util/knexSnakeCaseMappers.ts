// npm objection identifierMapping.js
...
export function knexSnakeCaseMappers(opt?: SnakeCaseMappersOptions) {
...
function knexIdentifierMappers({
  ...
  return {
    wrapIdentifier(identifier: string, origWrap: Identity<string>) {
...
function memoize(func: Identity<string>) {
  const cache = new Map();
...
function mapLastPart(mapper: Identity<string>, separator: string) {
  return (str: string) => {
    if (!str) return str;
...
function keyMapper(mapper: Identity<string>) {
  return (obj: object) => {
...
function camelCase(str: string, { upperCase = false } = {}) {
  if (str.length === 0) {
    return str;
  }
